...
public class Maquina {
    ...
    public void verificarUpdate(Integer idMaquina) {
        try {
            String sql = "SELECT m.sistemaOperacional, m.maxCpu, m.maxRam, m.maxDisco FROM maquina m WHERE idMaquina = ?";
            conec.queryForObject(sql, new Object[]{idMaquina}, (resposta, indice) -> {
                if (
                        !this.sistemaOperacional.equals(resposta.getString(1)) ||
                                this.maxCpu != resposta.getDouble(2) ||
                                this.maxRam != resposta.getDouble(3) ||
                                this.maxDisco != resposta.getDouble(4)
                ) {
                    conec.update("UPDATE maquina SET sistemaOperacional = ?, maxCpu = ?, maxRam = ?, maxDisco = ? WHERE idMaquina = ?",
                            this.sistemaOperacional, this.maxCpu, this.maxRam, this.maxDisco, idMaquina);
                    conec.update("""
                        UPDATE limitador SET limiteCpu = ?, limiteRam = ?, limiteDisco = ? WHERE fkMaquina = ?
                        """,getMaxCpu() * 0.8, getMaxRam() * 0.8, getMaxDisco() * 0.8, idMaquina);
                    System.out.println("""
                    *------------------------------------*
                    |              Sistema               |
                    *------------------------------------*
                    |Verificamos uma mudança no seu      |
                    |dispositivo, Atualizando ao banco   |
                    |de dados...                         
                    *------------------------------------*
                        """);
                }
            ...
            System.out.println("Máquina está atualizada");
        }
    }
